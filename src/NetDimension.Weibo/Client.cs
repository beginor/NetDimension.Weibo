...
namespace NetDimension.Weibo {
	...
	/// </summary>
...
#else
		public EntityInterfaces API { get; private set; }
#endif
...
#else
			this.API = new EntityInterfaces(this);
#endif
		}
		...
		public string PostCommand(string command, params WeiboParameter[] parameters) {
			return this.Http(command, RequestMethod.Post, parameters);
		}
		...
		private string Http(string command, RequestMethod method, params WeiboParameter[] parameters) {
			...
			return this.OAuth.Request(url, method, parameters);
		}
