...
namespace NetDimension.Weibo.Interface.Entity {
	public class AccountInterface : WeiboInterface {
		private readonly AccountAPI api;
		...
		public AccountInterface(Client client)
			: base(client) {
			this.api = new AccountAPI(client);
		}
		...
		public PrivacyEntity GetPrivacy() {
			return JsonConvert.DeserializeObject<PrivacyEntity>(this.api.GetPrivacy());
		}
		...
		public IEnumerable<SchoolEntity> SchoolList(string province = "", string city = "", string area = "", string type = "1", string capital = "", string keyword = "", int count = 10) {
			return JsonConvert.DeserializeObject<IEnumerable<SchoolEntity>>(this.api.SchoolList(province, city, area, type, capital, keyword, count));
		}
		...
		public RateLimitStatus RateLimitStatus() {
			return JsonConvert.DeserializeObject<RateLimitStatus>(this.api.RateLimitStatus());
		}
		...
		public string GetUID() {
			JObject result = JObject.Parse(this.api.GetUID());
			if (result["uid"] != null) {
		...
		public Entities.user.Entity EndSession() {
			return JsonConvert.DeserializeObject<Entities.user.Entity>(this.api.EndSession());
		}
		...
		public VerifyNickNameResult VerifyNickname(string nickname) {
			return JsonConvert.DeserializeObject<VerifyNickNameResult>(this.api.VerifyNickname(nickname));
		}
		...
		public UnreadCountResult UnreadCount(string uid, string callback = "") {
			return JsonConvert.DeserializeObject<UnreadCountResult>(this.api.UnreadCount(uid, callback));
		}
		...
		public bool SetCount(ResetCountType type) {
			JObject result = JObject.Parse(this.api.SetCount(type));
