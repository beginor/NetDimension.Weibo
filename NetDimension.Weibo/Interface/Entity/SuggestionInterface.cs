...
namespace NetDimension.Weibo.Interface.Entity {
	...
	public class SuggestionInterface : WeiboInterface {
		private readonly SuggestionAPI api;
		...
		public IEnumerable<Entities.user.Entity> HotUsers(HotUserCatagory category = HotUserCatagory.@default) {
			return JsonConvert.DeserializeObject<IEnumerable<Entities.user.Entity>>(this.api.HotUsers(category));
		}
		...
		public string MayInterestedUsers(int count = 10, int page = 1) {
			return this.api.MayInterestedUsers(count, page);
		}
		...
		public IEnumerable<Entities.user.Entity> UsersByStatus(string content, int num = 10) {
			return JsonConvert.DeserializeObject<IEnumerable<Entities.user.Entity>>(JObject.Parse(this.api.UsersByStatus(content, num))["users"].ToString());
		}
		...
		public IEnumerable<Entities.status.Entity> HotStatuses(int type = 1, bool isPic = false, int count = 20, int page = 1) {
			return JsonConvert.DeserializeObject<IEnumerable<Entities.status.Entity>>(JObject.Parse(this.api.HotStatuses(type, isPic, count, page))["statuses"].ToString());
		}
		...
		public IEnumerable<Entities.status.Entity> ReorderStatuses(int section, int count = 50, int page = 1) {
			return JsonConvert.DeserializeObject<IEnumerable<Entities.status.Entity>>(JObject.Parse(this.api.ReorderStatuses(section, count, page))["statuses"].ToString());
		}
		...
		public IDs ReorderStatusIDs(int section, int count = 50, int page = 1) {
			return JsonConvert.DeserializeObject<IDs>(this.api.ReorderStatusIDs(section, count, page));
		}
		...
		public IEnumerable<Entities.status.Entity> HotFavorites(int count = 20, int page = 1) {
			return JsonConvert.DeserializeObject<IEnumerable<Entities.status.Entity>>(this.api.HotFavorites(count, page));
		}
		...
		public Entities.user.Entity NotInterestedUsers(string uid) {
			return JsonConvert.DeserializeObject<Entities.user.Entity>(this.api.NotInterestedUsers(uid));
		}
	}
}
